...
namespace PredatorPrey
{
    ...
    class Simulation
    {
        ...
        private void CreateNewWarren()
        {
            ...
            } while (Landscape[x, y].Warren != null && Landscape[x,y].Terrain != 'L');
            if (ShowDetail)
            {
        ...
        private void CreateNewFox()
        {
            ...
            do
            {
                x = Rnd.Next(0, LandscapeSize);
                y = Rnd.Next(0, LandscapeSize);
            } while (Landscape[x, y].Fox != null && Landscape[x,y].Terrain != 'L');
            if (ShowDetail)
            {
        ...
        private bool CheckIfPathCrossesRiver(int WarrenX, int WarrenY, int FoxX, int FoxY)
        {
            bool doesCross = false;
            int distanceX = WarrenX - FoxX;
            int distanceY = WarrenY - FoxY;
            if (distanceX < 0)
            {
                for (int a = FoxX -1; a > WarrenX; a--)
                {
                    if (Landscape[a, FoxY].Terrain == 'R')
                    {
                        doesCross = true;
                        break;
                    }
            ...
            for (int FoxX = 0; FoxX < LandscapeSize; FoxX++)
            {
                for (int FoxY = 0; FoxY < LandscapeSize; FoxY++)
                {
                    if (Landscape[FoxX, FoxY].Fox != null)
                    {
                        ...
                        if (CheckIfPathCrossesRiver(WarrenX, WarrenY, FoxX, FoxY))
                        {
                            PercentToEat = 0;
                        }
                        else if (Dist <= 3.5)
